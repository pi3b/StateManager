function StateHandle(so){//状态处理函数，固定格式
	...
	case "":	//如果状态为空
		so.SetNextState("新建MQTT服务器",0,"");//那么将在下一扫描周期进入状态：访问西门子PLC
		break;	
	...
	case "访问Http":
		...
		break;
	case "新建MQTT服务器":
		var mqtt=so.Manager.GetSOObject("MQTT");
		alert(mqtt.ServerPort);
		if(mqtt.ServerActive == false)
		{
			mqtt.ServerStart();
			mqtt.Connect("127.0.0.1");
			mqtt.Subscribe("主题123");
			mqtt.Publish("主题123","第一条消息");
		}
		so.SetNextState("测试MQTT客户端");
		break;
	case "测试MQTT客户端":
		var mqtt=so.Manager.GetSOObject("MQTT");
		mqtt.Publish("主题123","定时消息456");
		so.RepeatState(3000);
		break;
	}
}
